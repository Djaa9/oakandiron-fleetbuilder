...
const squadronProvider = {
  SaveAndGetId: async (squadron) => {
    ...
    shortForm.ships = squadron.ships
      ? squadron.ships.map((ship) => {
          return {
            ...
            upgradeCards: [
              ship.upgradeCard1.name,
              ship.upgradeCard2.name,
            ].filter((card) => card),
            upgrades: ship.upgrades
              .map((upgrade) => {
                return upgrade.selected ? upgrade.name : null;
              })
              .filter((upgrade) => upgrade),
          };
        })
      : [];
    shortForm.initiativeCards = squadron.initiativeCards
      ? squadron.initiativeCards.map((card) => card.name)
      : null;
  ...
  GetFromId: async (id) => {
    ...
    if (shortForm.gameMode) {
      squadron.gameMode = gameModes.find(
        (mode) => mode.name === shortForm.gameMode
      );
    }
    if (shortForm.faction) {
      squadron.faction = factions.find(
        (faction) => faction.name === shortForm.faction
      );
    }
    if (shortForm.admiral) {
      squadron.admiral = allAdmirals.find(
        (admiral) => admiral.name === shortForm.admiral
      );
    }
    if (shortForm.ships) {
      squadron.ships = shortForm.ships.map((shipShortForm) => {
        // Copy ship and upgrades to new objects
        let newShip = shipProvider
          .all()
          .find((ship) => ship.name === shipShortForm.name);

        newShip.isFlagship = shipShortForm.isFlagship;
        newShip.commander = shipShortForm.commander
          ? Commanders.all().find(
              (commander) => commander.name === shipShortForm.commander
            )
          : "";
        newShip.skillLevel = newShip.skillUpgrades.find(
          (skillUpgrade) => skillUpgrade.name === shipShortForm.skillLevel
        );
        newShip.upgradeCard1 = upgradeCardsProvider
          .all()
          .find(
            (upgradeCard) => upgradeCard.name === shipShortForm.upgradeCards[0]
          );
        newShip.upgradeCard2 = upgradeCardsProvider
          .all()
          .find(
            (upgradeCard) => upgradeCard.name === shipShortForm.upgradeCards[1]
          );
        newShip.upgrade = shipShortForm.upgrades.forEach((upgradeShortForm) => {
          newShip.upgrades.find(
            (upgrade) => upgrade.name === upgradeShortForm
          ).selected = true;
        });
        return newShip;
      });
    ...
    if (shortForm.initiativeCards) {
      squadron.initiativeCards = shortForm.initiativeCards.map(
        (initiativeCardShortForm) => {
          return initiativeCardsProvider
            .all()
            .find((card) =>
              card.name === initiativeCardShortForm
                ? initiativeCardShortForm
                : ""
            );
        }
  ...
  toText: (squadron) => {
    ...
    text +=
      "GAME SIZE: " + (squadron.gameMode ? squadron.gameMode.name : "-") + "\n";
    text +=
      "FACTION: " + (squadron.faction ? squadron.faction.name : "-") + "\n";
    text +=
      "ADMIRAL: " + (squadron.admiral ? squadron.admiral.name : "-") + "\n";
    text += "\n";
    text += "SHIPS: \n\n";
    ...
    if (squadron.ships)
      squadron.ships.forEach((ship) => {
        text +=
          "# " + ship.name + (ship.isFlagship ? " (Flagship)" : "") + "\n";
        text += ship.commander ? "- " + ship.commander.name + "\n" : "";
        text += ship.skillLevel ? "- " + ship.skillLevel.name + "\n" : "";
        text += ship.upgrades.filter((upgrade) => upgrade.selected).length
          ? "Upgrades: \n"
          : "";
        ...
        text +=
          ship.upgradeCard1 || ship.upgradeCard2 ? "Upgrade Cards: \n" : "";
