...
function InitiativeCardSelector(props) {
  ...
  useEffect(() => {
    util.enforceHandRestrictions(availableInitiativeCards, admiral, faction, maxHandSize);
  },[availableInitiativeCards]);

  const handleListItemClick = (cardToAdd) => {  
    setAvailableInitiativeCards(TryToAddCardAndEnforceHandRestrictions(cardToAdd));
  };

  const TryToAddCardAndEnforceHandRestrictions = (cardToAdd) => {
    cardToAdd.selected = !cardToAdd.selected;

    let legalHand = availableInitiativeCards.map(card => card.name === cardToAdd.name ? cardToAdd : card);
