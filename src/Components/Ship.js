...
function Ship(props) {
  ...
  useEffect(() => {
    ...
    if (faction) {
      const commandersForFaction = Commanders.allowed(faction);
      setAvailableCommanders(commandersForFaction);
    }
    /* Set slected property of all upgrades to false*/
    const unselectedUpgrades = upgrades.map((upgrade) => {
    ...
    const availableUpgradeCards = UpgradeCards.allowed(
      faction,
      ship,
      isFlagship
    );
    setAvailableUpgradeCards(availableUpgradeCards);
  }, [faction, isFlagship, ship, upgrades]);

  // Calculate new cost of ship when selections change
  useEffect(() => {
    let newCostOfShip = ship.cost;
    newCostOfShip = selectedCommander
      ? selectedCommander.cost + newCostOfShip
      : newCostOfShip;
    newCostOfShip = selectedSkillLevel.cost
      ? newCostOfShip + selectedSkillLevel.cost
      : newCostOfShip;
    newCostOfShip = selectedUpgradeCard1.cost
      ? newCostOfShip + selectedUpgradeCard1.cost
      : newCostOfShip;
    newCostOfShip = selectedUpgradeCard2.cost
      ? newCostOfShip + selectedUpgradeCard2.cost
      : newCostOfShip;
    ...
    selectedCommander,
    selectedSkillLevel.cost,
    selectedUpgradeCard1.cost,
    selectedUpgradeCard2.cost,
    ship.cost,
    upgrades,
  ]);
  ...
  }, [costIncludingUpgrades, costUpdated, ship.costIncludingUpgrades, ship.id]);
