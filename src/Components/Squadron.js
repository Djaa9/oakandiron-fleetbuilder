...
function Squadron(props) {
  ...
  const [selectedFaction, setSelectedFaction] = useState(() => {
    return squadron.faction ? squadron.faction : "";
  });
  const [selectedGameMode, setSelectedGameMode] = useState(() => {
    return squadron.gameMode ? squadron.gameMode : "";
  });
  const [selectedAdmiral, setSelectedAdmiral] = useState(() => {
    return squadron.admiral ? squadron.admiral : "";
  });
  const [availableAdmirals, setAvailableAdmirals] = useState(() => {
    return squadron.admiral ? Admirals.allowed(squadron.faction) : [];
  });
  ...
  const [selectedInitiativeCards, setSelectedInitiativeCards] = useState(() => {
    return squadron.initiativeCards ? squadron.initiativeCards : [];
  });
  ...
  useEffect(() => {
    if (squadron) {
      ...
      if (squadron.admiral) {
        setSelectedAdmiral(
          newAvailableAdmirals.find(
            (admiral) => admiral.name === squadron.admiral.name
          )
        );
      }
  ...
  const handleShipChanged = (updatedShip) => {
    ...
    if (
      updatedShip.isFlagship &&
      selectedShips.filter((ship) => ship.isFlagship).length > 1
    ) {
      updatedListOfSelectedShips.map((ship) => {
        if (ship.id !== updatedShip.id) ship.isFlagship = false;
  ...
  return (
    <div className={classes.root}>
      ...
      >
        ...
        <Grid container className={classes.shipContainer} spacing={2}>
          {selectedShips.map((selectedShip) => (
            <Grid
              key={selectedShip.id.toString()}
              item
              xs={12}
              sm={6}
              md={4}
